// tb_riscv_singlecycle_loop.v - Verilog mirror of
// tb_riscv_singlecycle_loop.vhd. Counted decrement loop;
// expected: t0 = 0, t1 = 5.

`timescale 1ns/1ps
`default_nettype none

module tb_riscv_singlecycle_loop;
    ...
    initial begin
        $dumpfile(`FST_OUT);
        $dumpvars(0, tb_riscv_singlecycle_loop);
    end
    ...
    initial begin
        ...
        if (shadow_regs[5] !== 32'h00000000)
            $fatal(1, "prog_loop: expected t0 = 0, got %0d", shadow_regs[5]);
        if (shadow_regs[6] !== 32'h00000005)
            $fatal(1, "prog_loop: expected t1 = 5, got %0d", shadow_regs[6]);

        $display("tb_riscv_singlecycle_loop simulation done!");
        sim_active = 1'b0;
        $finish;
    end
