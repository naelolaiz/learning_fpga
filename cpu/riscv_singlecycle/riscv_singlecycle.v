// riscv_singlecycle.v - Verilog mirror of riscv_singlecycle.vhd.
//
// See the VHDL twin's header comment for the datapath outline,
// memory model, and debug-bus rationale. Behaviour is bit-equivalent
// — the same hex program runs to the same final state on either
// implementation.
//
// Verilog-specific notes:
//   * IMEM init uses $readmemh at elaboration; matches the VHDL
//     `init_imem` textio reader. Unprogrammed slots default to NOP
//     (`addi x0,x0,0` = 0x00000013).
//   * Internal IMEM is a `reg` array, not declared `constant`.
//     yosys infers a ROM from it (no write driver) just as it
//     handles the VHDL `constant`.
//   * DMEM mirrors the VHDL: sync write, async read.
...
module riscv_singlecycle #(
    ...
    initial begin
        ...
        if (IMEM_INIT != "") begin
            // Two-pass: count the file's words, then $readmemh with
            // an explicit end address. The single-arg form would
            // request the whole memory range and warn "not enough
            // words" on every program shorter than IMEM_DEPTH, which
            // is essentially every program. Counting first turns
            // that warning into the partial-load it really is.
    ...
    integer j;
    initial for (j = 0; j < DMEM_DEPTH; j = j + 1) dmem[j] = 32'b0;
    ...
    // ---------------------------------------------------------------
    // ID — decoder, immediate generator, regfile read
    // ---------------------------------------------------------------
    ...
    // Branch comparator — same 6-way table as the VHDL twin.
    always @(*) begin
        case (instr_funct3)
